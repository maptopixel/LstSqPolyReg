...
public final class Geometry
{
	...
	//Broken
	public static double jComputeSignedAngle(double ax, double ay,double bx,double by){
		double signed_angle = Math.atan2(by,bx) - Math.atan2(ay,ax);
		return signed_angle;
	}
	...
	public static int jGetQuadrant(double originX,double originY,double pX,double pY){
		int ans = -1;
	...
	public static boolean jOnRight(double originX,double originY,double pX,double pY){
		boolean ans;
		if (pX >= originX) {
			ans =  true;
		}else{
	...
	public static int jGetQuadrantInv(double originX,double originY,double pX,double pY){
		int ans = -1;
		if (pX >= originX && pY > originY) {
			ans =  1;
		}
	...
	public static int jGetQuadrantCorrection(double originX,double originY,double pX,double pY){
		int ans = -1;
		if (pX >= originX && pY > originY) {
			ans =  270;
		}
	...
	//JFR - What the hell does this function do again? 
	public static double jWhereOnLine(double queryPointX,double queryPointY,double linePoint1X,double linePoint1Y,double linePoint2X,double linePoint2Y){
		
		double value = jDeterminant(linePoint1X-queryPointX,linePoint2Y-queryPointY,linePoint1Y-queryPointY,linePoint2X-queryPointX);
		return value;
	}
	public static double jDeterminant(double m00,double m11, double m01, double m10){
		//Log.d("JFR","m00 " + m00 + " m11 " + m11 + " m01 " + m01 + " m10 " + m10);
		
		double value = m00 * m11 - m01 * m10;
		return value;
	...
	public static double bearing(double x1, double y1, double x2, double y2)
	{
	    ...
	    double angle = Math.toDegrees(Math.atan2(x1 - x2, y2 - y1));
     ...
     */
    public static double[] closestPointToSegment(double p1X,double p1Y,double p2X,double p2Y,double p3X,double p3Y) {

		final double xDelta = p2X - p1X;
		final double yDelta = p2Y - p1Y;

		if ((xDelta == 0) && (yDelta == 0)) {
			throw new IllegalArgumentException("p1 and p2 cannot be the same point");
		}

		final double u = ((p3X - p1X) * xDelta + (p3Y - p1Y) * yDelta) / (xDelta * xDelta + yDelta * yDelta);

		final double[] closestPoint = {666,666};
		if (u < 0) {
			closestPoint[0] = p1X;
			closestPoint[1] = p1Y;
			...
			closestPoint[0] = p1X + u * xDelta;
			closestPoint[1]=  p1Y + u * yDelta;
			//System.out.println(String.format("POINT X: " + closestPoint.getX()));
			//System.out.println(String.format("POINT Y: " + closestPoint.getY()));
		}
